...
Read through [GitHub’s Markdown guide](https://help.github.com/articles/basic-writing-and-formatting-syntax/) to get an understanding for how Markdown’s syntax works. Pay attention in particular to what Markdown syntax looks like for headings, bold text, links, and lists.
...
- **Index Page**: Update `index.html` such that, instead of merely listing the names of all pages in the encyclopedia, users can click on any entry name to be taken directly to that entry’s page.
...
- **New Page**: Clicking “Create New Page” in the sidebar should take the user to a page where they can create a new encyclopedia entry.
  ...
  - Upon saving, the user should be redirected to the new entry’s page.
- **Edit Page**: On each entry page, the user should be able to click a link to edit the entry’s page.
  - The user should be taken to a page where the `textarea` is pre-populated with the existing Markdown content of the page (i.e., the existing content should be the initial `value` of the `textarea`).
...
- **Markdown to HTML Conversion**: On each entry’s page, any Markdown content in the entry file should be converted to HTML before being displayed to the user. You may use the `markdown2` package to perform this conversion, installable via `pip install markdown2`.

## Hints

- To install `markdown2`, run `pip install markdown2`.
- For storing and retrieving encyclopedia entries, consider using Django’s built-in storage system.
